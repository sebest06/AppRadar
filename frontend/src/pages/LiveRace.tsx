...
const runnerIcon = (name: string) =>
  L.divIcon({
    html: `<div style="position:relative;display:flex;flex-direction:column;align-items:center;gap:2px">
      <div style="background:#16a34a;color:white;border-radius:50%;width:34px;height:34px;display:flex;align-items:center;justify-content:center;font-size:17px;border:2.5px solid white;box-shadow:0 2px 8px rgba(0,0,0,.25)">🏃</div>
      <div style="background:#0f172a;color:white;border-radius:4px;padding:1px 5px;font-size:10px;font-weight:600;white-space:nowrap;max-width:80px;overflow:hidden;text-overflow:ellipsis">${name}</div>
    </div>`,
    className: '',
    iconSize: [34, 50],
    iconAnchor: [17, 17],
...
function LeaderboardPanel({ rankings, positions }: { rankings: RankingEntry[]; positions: Map<string, LivePosition> }) {
  return (
    <div className="h-full overflow-y-auto">
      {rankings.length === 0 ? (
        ...
        <div className="space-y-2 p-4">
          {rankings.map((r, i) => {
            const live = positions.get(r.userUuid)
            ...
            return (
              <div
                ...
                className={`rounded-xl border p-3 transition-all ${
                  live ? 'border-green-200 bg-green-50' : 'border-slate-100 bg-slate-50'
                }`}
              >
...
export default function LiveRace() {
  ...
  useEffect(() => {
    if (!id) return
    const fetch = () => rankingsApi.get(id).then((r) => setRankings(r.data)).catch(() => {})
    fetch()
    timer.current = setInterval(fetch, 30_000)
    return () => { if (timer.current) clearInterval(timer.current) }
  }, [id])

  useEffect(() => {
    if (!id || !token) return
    const onPos = (p: LivePosition) => { setPositions((m) => new Map(m).set(p.userUuid, p)); setConnected(true) }
    const onRank = (r: RankingEntry[]) => setRankings(r)
    onPositionUpdate(onPos); onRaceUpdate(onRank); joinRace(id, token)
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)]">
      ...
      <div className="flex-shrink-0 bg-white border-b border-slate-100 px-4 sm:px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-3 flex-wrap">
          ...
          <span className={`flex-shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${
            ...
            {connected ? `${positions.size} en vivo` : 'Sin señal'}
          </span>
      ...
      <div className="flex-1 overflow-hidden lg:grid lg:grid-cols-3 lg:gap-0">
        {/* Map — full screen on mobile when tab=map */}
        <div className={`lg:col-span-2 ${tab === 'board' ? 'hidden lg:block' : 'block'} h-full`}>
          <MapContainer center={center} zoom={12} style={{ height: '100%', width: '100%' }} zoomControl={false}>
            ...
            {Array.from(positions.values()).map((pos) => (
              <Marker key={pos.userUuid} position={[pos.lat, pos.lon]} icon={runnerIcon(pos.userName.split(' ')[0])}>
                <Popup>
                  <div className="text-sm">
                    <p className="font-bold">{pos.userName}</p>
                    <p className="text-slate-500">{pos.teamName}</p>
                    <p className="text-slate-400 text-xs mt-1">Actualizado hace {timeAgo(pos.timestamp)}</p>
                  </div>
                </Popup>
              </Marker>
            ))}
        ...
        {/* Leaderboard sidebar */}
        <div className={`lg:block lg:border-l lg:border-slate-100 lg:bg-white overflow-hidden ${tab === 'map' ? 'hidden' : 'block'} h-full`}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 hidden lg:flex">
            <h2 className="font-bold text-slate-900 text-sm">Clasificación</h2>
            <span className="text-xs text-slate-400">{positions.size} activos</span>
          </div>
          <div className="h-full overflow-y-auto">
            <LeaderboardPanel rankings={rankings} positions={positions} />
          </div>
        </div>
      </div>
    </div>
